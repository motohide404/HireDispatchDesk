      ...
      <div className="w-full h-full p-4 bg-slate-50">
        ...
        <section id="driver-ledger" className="driver-ledger-section">
          <div className="driver-ledger-header">
            <h2>ドライバー台帳</h2>
            <p>
              現在の割当状況と延長使用回数（当月の延長利用累計/上限7回）を確認できます。各カードは当日の予約情報からリアルタイムに集計され、スクロール後もアンカー位置がわかるようにマージンを調整しています。
            </p>
          </div>
            ...
            {driverSummaries.map(({ driver, assignments }) => (
              <article key={driver.id} className="driver-card">
                ...
                <dl className="driver-card__meta">
                  ...
                  <div className="driver-card__row">
                    <dt className="driver-card__label">延長使用</dt>
                    <dd className="driver-card__value">
                      <span className="driver-card__highlight">{driver.extUsed}</span> / 7 回（当月累計）
                    </dd>
                  </div>
                </dl>
              </article>
